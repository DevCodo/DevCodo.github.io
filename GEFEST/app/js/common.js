...
// плавный переход по ссылкам в меню
  ...
  $(".section1__menu li a").mPageScroll2id();
...
// рандом на квиз
  ...
  let linkQuiz = document.querySelectorAll('.section1__hero__button_quiz_link');
  let linkRandom = Math.floor(Math.random()*4);

  if (linkRandom == 0) linkQuiz[0].removeAttribute('hidden');
  else if (linkRandom == 1) linkQuiz[1].removeAttribute('hidden');
  else if (linkRandom == 2) linkQuiz[2].removeAttribute('hidden');
  else if (linkRandom == 3) linkQuiz[3].removeAttribute('hidden');
